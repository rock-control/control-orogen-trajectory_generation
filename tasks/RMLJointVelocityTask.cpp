...
RTT::FlowStatus RMLVelocityPositionTask::getCurrentState(CurrentStateData &current_state){
...
RTT::FlowStatus RMLVelocityPositionTask::getTarget(TargetData& target_vector){
        ...
        for(size_t i = 0; i < target.size(); i++){
            try{
                if(!target[i].hasSpeed()){
                    LOG_ERROR("Target element %s does not have a valid speed value!", target.names[i].c_str());
...
void RMLVelocityPositionTask::writeCommand(const RMLOutputParameters& new_output_parameters){
