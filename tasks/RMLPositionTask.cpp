...
ReflexxesResultValue RMLPositionTask::performOTG(base::commands::Joints &current_command)
{
    ...
    *rml_input_parameters->CurrentAccelerationVector = *rml_output_parameters->NewAccelerationVector;
    ...
    for(size_t i = 0; i < motion_constraints.size(); i++){
        if(fabs(rml_input_parameters->CurrentVelocityVector->VecData[i]) < 1e-5)
            rml_input_parameters->CurrentVelocityVector->VecData[i] = 0;
        if(fabs(rml_input_parameters->CurrentAccelerationVector->VecData[i]) < 1e-5)
            rml_input_parameters->CurrentAccelerationVector->VecData[i] = 0;
    }
