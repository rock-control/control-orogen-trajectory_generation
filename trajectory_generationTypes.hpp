...
struct MotionConstraint{
    ...
    double max_pos;  /** Maximum joint or Cartesian position. In rad or m. (only reflexxes TypeIV, has to be > min. position!) */
    double min_pos;  /** Minimum joint or Cartesian position. In rad or m. (only reflexxes TypeIV, has to be < max. position!) */
    double max_vel;  /** Maximum joint or Cartesian velocity. In rad/s or m/s. (has to be > 0) */
    double max_acc;  /** Maximum joint or Cartesian acceleration. In rad/ss or m/ss. (has to be > 0) */
    double max_jerk; /** Maximum joint or Cartesian jerk (derivative of acceleration). In rad/sss or m/sss. (has to be > 0) */

    MotionConstraint(){
        max_pos  = base::unset<float>();
        min_pos  = base::unset<float>();
        max_vel  = base::unset<float>();
        max_acc  = base::unset<float>();
        max_jerk = base::unset<float>();
    }

    bool hasMaxPosition() const {return !base::isUnset(max_pos);}
    bool hasMinPosition() const {return !base::isUnset(min_pos);}
    bool hasMaxVelocity() const {return !base::isUnset(max_vel);}
    bool hasMaxAcceleration() const {return !base::isUnset(max_acc);}
    ...
    void applyDefaultIfUnset(const MotionConstraint& default_constraints){
        if(!hasMaxPosition())
            max_pos = default_constraints.max_pos;
        if(!hasMinPosition())
            min_pos = default_constraints.min_pos;
        if(!hasMaxVelocity())
            max_vel = default_constraints.max_vel;
        if(!hasMaxAcceleration())
            max_acc = default_constraints.max_acc;
        if(!hasMaxJerk())
